...
import { ResearchArea, FacultyResearchArea, FacultySlot } from '../types';
import ResearchAreaSelector from '../components/ResearchAreaSelector';
import { Save, CheckCircle2, Users, BookOpen, SlidersHorizontal, Minus, Plus, FileText, Clock, Sparkles } from 'lucide-react';
...
export default function FacultyDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [areasData, slotData, myAreasData] = await Promise.all([
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        ...
        <section className="card p-6 animate-slide-up stagger-1 shadow-soft lg:col-span-2">
          ...
          <div className="mt-4 flex items-center justify-between rounded-xl bg-neutral-50 p-4">
            ...
            <span className={`text-2xl font-bold ${available > 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{available}</span>
          </div>
